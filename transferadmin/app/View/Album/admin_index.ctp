...
?>
 ...
 <div align="right">
<a class="btnIconLeft mt5" title="" href="<?php echo SITE_URL; ?>admin/album/add/"><img class="icon" alt="" src="<?php echo SITE_URL; ?>img/icons/dark/presentation.png"><span>Add New Album</span></a>
 </div>
	
<div class="table">
            <div class="head"><h5 class="iFrames">Album</h5></div>
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th rowspan="1" colspan="1"  width="2%" id="none" class="none;">
							...
							<?php echo $this->Form->text('Album.all', array('type'=>'checkbox', 'id'=>'data[Album][all]', 'style' => 'display:block;', 'onclick'=>"selectDeselect('data[Album][id][]', this.name);"));?>
				...
				<tr class="gradeA <?php if($rowcount%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center">
						...
						<?php echo $this->Form->text('Album.id][', array('type'=>'checkbox', 'style' => 'display:block;', 'value'=>$content[$rowcount]['Album']['id'],  'onclick'=>"checkSelection('data[Album][id][]', 'data[Album][all]')"));?>
						
					</td>
					<td align="center"><?php echo $index;?></td>
					<td><?php 
							echo $this->Html->link($content[$rowcount]['Album']['title'], '/admin/album/edit/'.$content[$rowcount]['Album']['id'].'/', array('title'=>'Click here to edit Album'));
					?></td>
					
					<td align="center"><?php echo date(DATE_FORMAT, strtotime($content[$rowcount]['Album']['created'])); ?></td>
					<td align="center">
					<?php
						 if($content[$rowcount]['Album']['status'] == 1) {
				...
				<div class="fg-toolbar ui-toolbar  ui-corner-bl ui-corner-br ui-helper-clearfix">
					<div id="example_length" class="dataTables_length">
						<?php
							echo $this->Form->input('Album.status', array(
								'options' => $mode,
								'empty' => '(action)',
								'label' => false,
						...
						?>
						&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="Submit" class="redBtn" onclick="return del('data[Album][id][]')" name="data[Album][submit]">
					</div>
