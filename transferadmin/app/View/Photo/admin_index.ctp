...
?>
 ...
 <div align="right">
<a class="btnIconLeft mt5" title="" href="<?php echo SITE_URL; ?>admin/photo/add/"><img class="icon" alt="" src="<?php echo SITE_URL; ?>img/icons/dark/presentation.png"><span>Add New Photo</span></a>
 </div>
...
<div class="table">
			...
			<table cellspacing="0" cellpadding="0" border="0" id="" class="display">
                <thead>
                    <tr>
						<th rowspan="1" colspan="1"  width="2%" id="none" class="none;">
							<?php echo $this->Form->text('Photo.all', array('type'=>'checkbox', 'id'=>'data[Photo][all]', 'style' => 'display:block;', 'onclick'=>"selectDeselect('data[Photo][id][]', this.name);"));?>
						</th>
						...
						<th class="ui-state-default" rowspan="1" colspan="1">
							<div class="DataTables_sort_wrapper">
								<?php echo $this->Paginator->sort('sub_title', 'Sub Title'); ?>
								<span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
							</div>
						</th>
				...
				<tr class="gradeA <?php if($rowcount%2==0) echo 'odd'; else echo 'even';?>">
					<td align="center">
						<?php echo $this->Form->text('Photo.id][', array('type'=>'checkbox', 'style' => 'display:block;', 'value'=>$row['Photo']['id'],  'onclick'=>"checkSelection('data[Photo][id][]', 'data[Photo][all]')"));?>
					</td>
					<td align="center"><?php echo $index;?></td>
					<td><?php 
							echo $this->Html->link($row['Photo']['title'], '/admin/photo/edit/'.$row['Photo']['id'].'/', array('title'=>'Click here to edit photo details'));
					?></td>
					<td align="left">
						<?php echo $row['Photo']['sub_title']; ?>
					</td>
					<td align="left">
						<img src="<?php echo SITE_URL; ?>files/photo/<?php echo $row['Photo']['photo']; ?>" style="width:150px;">
					</td>
					<td align="center"><?php echo date(DATE_FORMAT, strtotime($row['Photo']['created'])); ?></td>
					<td align="center">
					<?php
						 if($row['Photo']['status'] == '1') {
				...
				<div class="fg-toolbar ui-toolbar  ui-corner-bl ui-corner-br ui-helper-clearfix">
					<div id="example_length" class="dataTables_length">
						<?php
							echo $this->Form->input('Photo.status', array(
								'options' => $mode,
								'empty' => '(action)',
								'label' => false,
						...
						?>
						&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="Submit" class="redBtn" onclick="return del('data[Photo][id][]')" name="data[Photo][submit]">
					</div>
