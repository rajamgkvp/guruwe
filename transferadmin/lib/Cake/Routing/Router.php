...
######### CODE ADDED BY RISHABH FOR URL DECRYPTION #########	
	...
	public static function encode($value){ 
		...
		$crypttext = mcrypt_encrypt(MCRYPT_RIJNDAEL_256, 'RISHABH', $text, MCRYPT_MODE_ECB, $iv);
    ...
    public static function decode($value){
       if(!$value){return false;}
        ...
        $iv_size = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB);
        $iv = mcrypt_create_iv($iv_size, MCRYPT_RAND);
        $decrypttext = mcrypt_decrypt(MCRYPT_RIJNDAEL_256, 'RISHABH', $crypttext, MCRYPT_MODE_ECB, $iv);
        return trim($decrypttext);
    }	
	
	
	public static function spEncrypt($v) {
		$request_url = '';
		foreach($_REQUEST as $key=>$value) {
			$request_url = $key;
			break;
		}
		if(substr($request_url,0,7)=='/admin/') {
			return $v;
		}
		return self::encode($v);
	}
	
	public static function spDecrypt($v) {
		//If string lenght is less than 10 character then it is not encrypted, so no need to decrypt it.
		if(strlen($v)<11) {
			return $v;
		} else {
